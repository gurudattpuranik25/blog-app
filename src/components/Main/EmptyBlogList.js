...
function EmptyBlogList({ isAuth }) {
  return (
    <div className=" empty__blog flex justify-center items-center gap-8 mt-8">
      <img
        src={emptyBlogList}
      ...
      <div className=" flex flex-col gap-4 items-center">
        <h1 className=" empty__tag text-2xl font-semibold">
          Oops !! No blogs under this category at the moment.
        </h1>
        <Link
          to={`/${isAuth ? "create" : "login"}`}
          className=" create__btn flex items-center bg-white gap-2 border-2 px-4 py-1 rounded-lg "
        >
          <AiOutlinePlus />
          Create New Post
        </Link>
